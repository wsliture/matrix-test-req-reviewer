...
const heading = (value: string, level: number, anchorId?: string, sourceRefs?: string[], businessId?: string): Phase2Block => ({type: "heading", text: value, level, anchorId, sourceRefs, businessId});
const paragraph = (value: unknown): Phase2Block => ({type: "paragraph", text: text(value)});
const table = (caption: string, columns: string[], rows: unknown[][]): Phase2Block => ({type: "table", caption, columns, rows: rows.map(row => row.map(item => text(item)))});
...
function compressRequirementIds(ids: string[]) {
    ...
    for (let index = 0; index < ids.length;) {
        ...
        if (!match) { result.push(first); index++; continue }
...
function chapter1(data: Json, artifact: string, lookup: ReturnType<typeof maps>): Phase2Block[] {
    const root = lookup.root(artifact), csci = list(data.csci_names).join("、"), documentId = text(data.document_id), renderedId = !documentId || documentId.endsWith(".RX1") ? documentId : `${documentId}.RX1`;
    const references = Array.isArray(data.references) ? data.references : [];
    return [
        heading("1 范围", 1, root?.id, refs(data), root?.businessId), heading("1.1 标识", 2),
...
function chapter2(data: Json, artifact: string, lookup: ReturnType<typeof maps>): Phase2Block[] {
    const root = lookup.root(artifact), blocks: Phase2Block[] = [heading("2 系统概述", 1, root?.id, refs(data), root?.businessId), heading("2.1 运行环境说明", 2)];
...
function parseDocxTableXml(xml: string) {
    type ParsedCell = {text: string; colSpan: number; rowSpan: number; column: number; merge?: "restart" | "continue"};
    const parsedRows: ParsedCell[][] = [...xml.matchAll(/<w:tr\b[^>]*>([\s\S]*?)<\/w:tr>/g)].map(row => {
        ...
        return [...row[1].matchAll(/<w:tc\b[^>]*>([\s\S]*?)<\/w:tc>/g)].map(cell => {
            const cellXml = cell[1], colSpan = Number(cellXml.match(/<w:gridSpan\b[^>]*w:val="(\d+)"/)?.[1] || 1),
                mergeTag = cellXml.match(/<w:vMerge\b([^>]*)\/?\s*>/)?.[1], merge = mergeTag === undefined ? undefined : /w:val="restart"/.test(mergeTag) ? "restart" : "continue",
                value: ParsedCell = {text: decodeXml([...cellXml.matchAll(/<w:t\b[^>]*>([\s\S]*?)<\/w:t>/g)].map(match => match[1]).join(""))
    ...
    for (let rowIndex = 0; rowIndex < parsedRows.length; rowIndex++) {
        for (const cell of parsedRows[rowIndex]) {
            ...
            for (let previous = rowIndex - 1; previous >= 0; previous--) {
                ...
                if (origin) { origin.rowSpan++; break }
            }
        }
    }
    ...
    const expandedRows = parsedRows.map(row => row.map(cell => cell.text)), width = Math.max(...expandedRows.map(row => row.length)), normalized = expandedRows.map(row => [...row, ...Array(Math.max(0, width - row.length)).fill("")]);
...
function hardware(data: Json, artifact: string, lookup: ReturnType<typeof maps>): Phase2Block[] {
    const root = lookup.root(artifact), blocks: Phase2Block[] = [heading(`3 ${text(data.chapter_title) || "数据及接口需求"}`, 1), heading(`3.1 ${text(data.section_title) || "硬件接口"}`, 2, root?.id, refs(data), root?.businessId)];
    (data.interfaces || []).forEach((item: Json, index: number) => {
        const number = `3.1.${index + 1}`, node = lookup.anchor(artifact, number), title = text(item.title || item.interface_name || item.name) || `硬件接口${index + 1}`;
        ...
        let tableNumber = 1;
        const appendTables = (tables: Json[], fallback: string) => { for (const candidate of tables || []) { const value = normalizedTable(candidate, fallback); if (value) { value.caption = `表3.1.${index + 1}-${tableNumber++}  ${cleanTableTitle(value.caption) || fallback}`; blocks.push(value) } } };
        const flowGroup = (flows: unknown, tables: Json[], label: string) => { for (const value of flowList(flows)) blocks.push(paragraph(value)); appendTables(tables, label) };
        appendTables(item.overview_tables, `${title}概述数据表`);
        flowGroup(item.input_flow, item.input_tables, `${title}输入数据表`); flowGroup(item.output_flow, item.output_tables, `${title}输出数据表`);
        (item.topics || []).forEach((sub: Json, subIndex: number) => { blocks.push(heading(`${number}.${subIndex + 1} ${text(sub.title || sub.name)}`, 4, undefined, refs(sub))); flowGroup(sub.input_flow, sub.input_tables, `${text(sub.title)}输入数据表`); flowGroup(sub.output_flow, sub.output_tables, `${text(sub.title)}输出数据表`) })
    });
...
function functional(data: Json, artifact: string, lookup: ReturnType<typeof maps>): Phase2Block[] {
    const root = lookup.root(artifact), blocks: Phase2Block[] = [heading(`${text(data.chapter_title_no) || "4"} ${text(data.chapter_title) || "测试类型说明"}`, 1), heading(`${text(data.section_title_no) || "4.1"} ${text(data.section_title) || "功能测试"}`, 2, root?.id, refs(data), root?.businessId)];
    const visit = (node: Json) => {
        const number = text(node.title_no), match = lookup.anchor(artifact, number), children = Array.isArray(node.children) ? node.children : [], content = node.init_content || node.other_content;
        blocks.push(heading(`${number} ${text(node.title)}`, Math.min(number.split(".").length, 5), match?.id, refs(node), match?.businessId));
        if (children.length) return children.forEach(visit);
        if (!content) return;
        if (text(content.summary)) blocks.push(paragraph(content.summary));
        blocks.push(heading(`${number}.1 输入流说明`, 5), {type: "list", items: formatFunctionalFlowItems(content.input_flow)}, heading(`${number}.2 处理`, 5));
        ...
        for (const processing of content.processing || []) {
            const requirementId = text(processing.requirement_id), target = lookup.anchor(artifact, undefined, requirementId);
            blocks.push({type: "paragraph", text: `${requirementId}：${text(processing.content)}`, anchorId: target?.id, businessId: requirementId, sourceRefs: refs(processing)});
        ...
        blocks.push(heading(`${number}.3 输出流说明`, 5), {type: "list", items: formatFunctionalFlowItems(content.output_flow)}, heading(`${number}.4 测试要求`, 5), paragraph("测试充分性要求：C1、C2；"), paragraph("测试终止要求：Z1、Z2；"), paragraph("优先级：所有需求项为低优先级。"))
    };
...
function nonFunctional(data: Json, artifact: string, number: string, title: string, lookup: ReturnType<typeof maps>): Phase2Block[] {
    const config = NON_FUNCTIONAL[number], root = lookup.root(artifact), rows = data.rows || [], sectionNo = text(data.section_title_no) || `${number}.1`, sectionTitle = text(data.section_title) || `${config.noun}需求项[${config.prefix}]`;
    ...
    return [heading(`${number} ${title}`, 2, root?.id, refs(data), root?.businessId), heading(`${sectionNo} ${sectionTitle}`, 3, sectionNode?.id, refs(data), sectionNode?.businessId), paragraph(`依据引用文档及上述测试需求分析，共总结如下 ${rows.length} 项${config.noun}需求：`), summary, heading(`${number}.2 测试要求`, 3), paragraph(`测试充分性要求：${text(data.test_requirements?.sufficiency)}；`), paragraph(`测试终止要求：${text(data.test_requirements?.termination)}；`), paragraph(`优先级：所有需求项为${text(data.test_requirements?.priority)}。`)]
}

function traceability(data: Json, artifact: string, lookup: ReturnType<typeof maps>): Phase2Block[] {
    const root = lookup.root(artifact), rows = data.rows || [], requirementCount = data.summary?.requirement_count ?? [...new Set(rows.flatMap((item: Json) => item.test_requirement_ids || []))].length;
...
export async function buildPhase2Document(workspacePath: string, requirements: any[]) {
    const dataDir = path.join(workspacePath, ".matrix", "data"), lookup = maps(requirements), chapters: Phase2Chapter[] = [];
    for (const [artifact, number, title] of FILES) {
        try {
            ...
            else if (number === "3.1") { await hydrateHardwareTables(data); blocks = hardware(data, artifact, lookup) }
            ...
            chapters.push({artifact, number, title, rootNodeId: lookup.root(artifact)?.id, blocks: [{type: "error", text: `${number} ${title}加载失败：${error instanceof Error ? error.message : String(error)}`}]})
        }
    }
    return {chapters}
}

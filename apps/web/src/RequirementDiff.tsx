...
export function RequirementDiffPage() {
    ...
    const options = revisions.data?.map(item => ({value: item.id, label: `${item.versionLabel}${item.kind === "MIGRATED_BASELINE" ? "（迁移基线）" : item.kind === "GENERATED_BASELINE" ? "（初始基线）" : ""}`}));
    ...
    return <div className="requirement-diff-page">
        ...
        <section className="requirement-diff-toolbar"><Space wrap><Input.Search placeholder="搜索TR编号或标题" allowClear value={search} onChange={event => setSearch(event.target.value)} className="diff-search"/>
            <Select allowClear placeholder="全部变更类型" value={kind} onChange={value => {setKind(value); setSelectedUid(undefined)}} options={Object.entries(LABEL).map(([value, label]) => ({value, label}))}/><span>仅显示变化章节</span><Switch checked={onlyChanged} onChange={setOnlyChanged}/>
